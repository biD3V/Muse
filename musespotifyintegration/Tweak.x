...
- (void)grabNextUp {
    ...
    // create missing
    NSDictionary *missingno = @{
        @"title" : @"No Songs",
        @"artist_name" : @"In Queue"
    };
    // Get the Metadata for the next track
    [userInfo setObject:self.nextTracks[0].metadata ?: missingno forKey:@"metadata"];
    // Send Metadata
...
%hook SPTRecentlyPlayedListImplementation
...
- (SPTRecentlyPlayedListImplementation *)initWithRecentlyPlayedModel:(id)arg1 maxNumberOfItems:(unsigned long long)arg2 filterPredicate:(id)arg3 {
    ...
    // Add image URLs and URIs to array
    ...
    NSError *error = nil;
    // Check for Muse directory and add it if it is not there
    ...
    [rawImageURLs writeToFile:rawPath atomically:NO]; // Save URLs and URIs for debuging
    ...
    for (SPTRecentlyPlayedEntity *entity in implementation.entities) {
        ...
        if (components.count > 2) { // Check if URL is actually an identifier
            if ([components[1] isEqualToString:@"mosaic"] && components.count > 3) { // Playlist
                ...
                [imageURLs addObject:[NSString stringWithFormat:@"https://mosaic.scdn.co/300/%@%@%@%@", components[2], components[3] ?: components[2], components[4] ?: components[2], components[5] ?: components[2]]];
